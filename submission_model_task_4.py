...
class Submission:
    ...
    def __init__(self):
        ...
        print(words)
        hidden_dim = int(words[3])
        question_dim = int(words[4])
        dropout = float(words[6])
        concat_dim = int(words[9])
        concat_hidden_dim = int(words[8])
        ...
        self.set_mask = None
        print("Loaded params:")
    def select_feature(self, masked_data, masked_binary_data, can_query,stu_features):
        """
        ...
        m = torch.nn.Sigmoid()
        selections = []
        stu_features = torch.from_numpy(deepcopy(stu_features)).long().to(device)
        ...
        with torch.no_grad():
            ...
            input_embedding = torch.cat([input_embedding,stu_features],1)########
    ...
    def predict(self, masked_data, masked_binary_data,stu_features):
        """
